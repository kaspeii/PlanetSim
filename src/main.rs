...
fn setup_globe(
    ...
    for _ in 0..NUM_PLATES {
        let plate_type = if rng.random_bool(PERC_OF_CONTINENTAL_PLATES) { PlateType::Continental } else { PlateType::Oceanic };
        plates.push(Plate {
            center: Vec3::new(rng.random_range(-1.0..1.0), rng.random_range(-1.0..1.0), rng.random_range(-1.0..1.0)).normalize(),
            plate_type,
            drift_dir: Vec3::new(rng.random_range(-1.0..1.0), rng.random_range(-1.0..1.0), rng.random_range(-1.0..1.0)).normalize(),
        });
    }
...
fn apply_tectonic_deformation(mesh: &mut Mesh, plates: &[Plate], noise: &impl NoiseFn<f64, 3>) {
    if let Some(VertexAttributeValues::Float32x3(positions)) = mesh.attribute_mut(Mesh::ATTRIBUTE_POSITION) {
        ...
        for pos in positions.iter_mut() {
            ...
            let mut h = if p1.plate_type == PlateType::Continental { 0.12 } else { -0.35 };
            ...
            else if p1.plate_type == PlateType::Continental && p2.plate_type == PlateType::Continental {
            ...
            if boundary_dist < edge_threshold {
                ...
                let mountain_noise = 0.6 + noise.get([v.x as f64 * 15.0, v.y as f64 * 15.0, v.z as f64 * 15.0]) as f32 * 1.2;
            ...
            let detail_noise = noise.get([v.x as f64 * 4.0, v.y as f64 * 4.0, v.z as f64 * 4.0]) as f32;
            h += detail_noise * 0.35;
